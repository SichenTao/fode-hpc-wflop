...
Implementation unit: resumable T24 Waffle H6 and six-case formal campaign
...
EXPECTED_IDS = {
    "uniform_p0": "t24_uniform_threshold_000_n25",
    "uniform_p007": "t24_uniform_threshold_007_n25",
    "uniform_p015": "t24_uniform_threshold_015_n25",
    "real_p0": "t24_real_threshold_000_n25",
    "real_p007": "t24_real_threshold_007_n25",
    "real_p015": "t24_real_threshold_015_n25",
}
METHOD_ID = "t24_nsga3_markov_intermittency_declared_reconstruction_v1"
PROBLEM_ID = "t24_kim_markov_intermittency_six_case_v1"
POPULATION = 92
GENERATIONS = 1000
PHYSICAL_FES = POPULATION * (GENERATIONS + 1)
...
def validate(payload: dict, case_name: str, workers: int) -> None:
    ...
    require(
        ...
        and payload.get("generations") == GENERATIONS
    ...
    for point in payload["front"]:
        require(
            math.isfinite(point["mean_power_mw"])
            and point["mean_power_mw"] > 0.0,
            f"{case_name}: nonfinite power",
        )
        require(
            math.isfinite(point["intermittency_mw"])
            and point["intermittency_mw"] >= 0.0,
            f"{case_name}: nonfinite intermittency",
        )
...
def run_h6(args: argparse.Namespace, root: Path) -> dict:
    ...
    for workers in (1, args.total_workers):
        ...
        validate(rows[workers], "real_p0", workers)
    serial = rows[1]
    parallel = rows[args.total_workers]
    require(
        serial["scientific_hash"] == parallel["scientific_hash"],
        "T24 one/all-core scientific trajectory mismatch",
    )
    speedup = {
        "evaluator":
    ...
    require(
        ...
        f"T24 dominant stages did not accelerate: {speedup}",
    )
    result = {
        "status": "pass",
        "case": "real_p0",
...
def summarize_case(payloads: list[dict]) -> dict:
    maximum_power = [
        max(point["mean_power_mw"] for point in item["front"])
        for item in payloads
    ]
    minimum_intermittency = [
        min(point["intermittency_mw"] for point in item["front"])
        for item in payloads
    ]
    return {
        ...
        "mean_maximum_front_power_mw": statistics.fmean(maximum_power),
        "standard_deviation_maximum_front_power_mw":
            statistics.stdev(maximum_power),
        "mean_minimum_front_intermittency_mw":
            statistics.fmean(minimum_intermittency),
        "standard_deviation_minimum_front_intermittency_mw":
            statistics.stdev(minimum_intermittency),
...
def main() -> int:
    ...
    parser.add_argument("--seed-base", type=int, default=2026024000)
    args = parser.parse_args()
    require(args.total_workers == 20, "T24 Waffle requires all 20 cores")
    require(args.repeat_count == 25, "T24 formal requires 25 seeds")
    ...
    for case_index, case_name in enumerate(CASES):
        ...
        for repeat in range(args.repeat_count):
            ...
            completed_runs += 1
            print(
                f"t24_formal_progress completed={completed_runs}/"
    ...
    summary = {
        ...
        "claim_boundary":
            ...
            "figure-constrained academic reproduction; not author source, "
            "original Marado data, private arrays, or numerical replay",
    }
