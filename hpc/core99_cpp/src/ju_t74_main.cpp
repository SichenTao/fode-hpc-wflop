...
Implementation unit: T74 CLI and machine-readable H5/H6/formal receipts
Paper/source/conflict/reconstruction:
...
struct Arguments {
    ...
    std::string case_id = "t74_case1_medium";
...
Arguments parse(const int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (flag == "--mode") result.mode = value();
            ...
            throw std::invalid_argument("unknown T74 flag " + flag);
        }
    }
    return result;
}
...
core99::t74::Layout parse_layout(const std::string& value) {
    if (value.empty()) return core99::t74::regular_reference_layout();
    core99::t74::Layout result;
...
std::string evaluation_json(const core99::t74::Evaluation& value) {
    std::ostringstream output;
    output << std::setprecision(17)
...
std::string run_json(const core99::t74::RunResult& value) {
    std::ostringstream output;
    output << std::setprecision(17)
...
void emit(const std::string& content, const std::string& path) {
    ...
    if (!stream) throw std::runtime_error("cannot open T74 output");
    stream << content;
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (arguments.mode == "list-cases") {
            emit(
                "{\"paper_case_ids\":"
                    + string_vector_json(core99::t74::paper_case_ids())
                    + "}\n",
                arguments.output
            );
            return EXIT_SUCCESS;
        }
        const core99::t74::Problem problem(arguments.case_id);
        if (arguments.mode == "inspect") {
            std::ostringstream output;
            output << std::setprecision(17)
                << "{\"case_id\":\"" << problem.case_id()
                << "\",\"problem_semantic_id\":\"" << problem.semantic_id()
                << "\",\"wind_case\":" << problem.wind_case()
                << ",\"cell_width_m\":" << problem.cell_width_m()
                << ",\"wind_state_count\":" << problem.wind_state_count()
                << ",\"candidate_count\":441,\"turbine_count\":60"
        ...
        if (arguments.mode == "evaluate") {
            emit(
                evaluation_json(problem.evaluate(parse_layout(arguments.indices)))
                    + "\n",
                arguments.output
            );
            return EXIT_SUCCESS;
        ...
        if (arguments.mode != "optimize" && arguments.mode != "formal") {
            throw std::invalid_argument("unknown T74 mode " + arguments.mode);
        }
        if (arguments.repeats < 1) {
            throw std::invalid_argument("T74 repeats must be positive");
        }
        std::ostringstream output;
        output << "{\"mode\":\"" << arguments.mode << "\",\"runs\":[";
        for (int repeat = 0; repeat < arguments.repeats; ++repeat) {
            core99::t74::RunConfig config;
        ...
        std::cerr << "core99_t74_hpc: " << error.what() << '\n';
        return EXIT_FAILURE;
    }
}
