...
Implementation unit: L0371 CLI and machine-readable H5/H6/formal receipts
...
struct Arguments {
    ...
    std::string case_id = "l0371_ideal_a_n";
    std::string data;
    std::string output;
    std::string indices;
    int workers = 20;
...
Arguments parse(const int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (flag == "--mode") arguments.mode = value();
            ...
            throw std::invalid_argument("unknown L0371 flag: " + flag);
        }
    }
...
std::string evaluation_json(const core99::l0371::Evaluation& value) {
    std::ostringstream output;
    output << std::setprecision(17)
        << "{\"coe\":" << value.coe
        << ",\"average_power_kw\":" << value.average_power_kw
        << ",\"no_wake_power_kw\":" << value.no_wake_power_kw
...
void emit(const std::string& content, const std::string& path) {
    ...
    if (!stream) throw std::runtime_error("cannot open L0371 output");
    stream << content;
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (arguments.mode == "list-cases") {
            emit(
                "{\"paper_case_ids\":"
                    + vector_json(core99::l0371::paper_case_ids())
                    + "}\n",
                arguments.output
            );
            return EXIT_SUCCESS;
        }
        const core99::l0371::Problem problem(
        ...
        if (arguments.mode == "inspect") {
            ...
            output << std::setprecision(17)
                ...
                << problem.semantic_id() << "\",\"horns\":"
                << (problem.is_horns() ? "true" : "false")
                << ",\"candidate_count\":" << problem.candidates().size()
                << ",\"turbine_count\":" << problem.turbine_count()
                << ",\"state_count\":" << problem.states().size()
                << ",\"state_probability_sum\":" << probability_sum
                << ",\"minimum_spacing_m\":"
        ...
        if (arguments.mode != "optimize") {
            throw std::invalid_argument("unknown L0371 mode");
        }
        core99::l0371::RunConfig config;
        ...
        output << std::setprecision(17)
            ...
            << "  \"best_power_history_kw\":"
            << vector_json(result.best_power_history_kw) << ",\n"
            << "  \"precomputation_seconds\":"
        ...
        std::cerr << "L0371 error: " << error.what() << '\n';
        return EXIT_FAILURE;
    }
}
